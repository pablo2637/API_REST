document.addEventListener('DOMContentLoaded', () => {
    ...
    if (nav)
        nav.addEventListener('click', ev => {
            ...
            if (ev.target.matches('div') && ev.ctrlKey && ev.altKey && ev.shiftKey)
    ...
    main.addEventListener('click', ev => {
        ...
        else if (ev.target.parentNode.matches('td') && ev.target.matches('img') && ev.target.baseURI.includes('dashboard'))
    ...
    const addToCart = async (id, tipo) => {
        ...
        if (ok) {
            ...
            if (indProd != -1)
                arrayCarrito[indProd].cantidad += 1;
            ...
            console.log('Error addToCart', resp);
